...
def main():
    ...
    availableTypes = ["ALL", "1987_NC", "1987_C"]

    if simulationType == None or (simulationType not in availableTypes):
        print "requested type missing (ty) or invalid Please select from the following:"
        print "ALL - performs all simulations"
        print "1987_NC - performs non-compounding simulations"
        print "1987_C - performs compounding simulations"
        sys.exit()
    ...
    if shift == None:
        print "no shift specified, assuming non-shifted TF."
        shift = 0
    else:
        shift = int(shift)
...
""")
    ...
    if simulationType == "ALL" or simulationType == "1987_NC" or simulationType == "1987_C":
        ...
        if shift == 0:
            ...
            replace(os.path.join(os.getcwd(), 'config', 'ast_kantu_1987.config'), "pair =", "pair =" + symbol + "1987_" + "S" + str(shift))
    ...
    if simulationType == "ALL" or simulationType == "1987_C":
    ...
    if simulationType == "ALL" or simulationType == "1987_NC":
