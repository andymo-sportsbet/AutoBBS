...
LinearRegression1::LinearRegression1(Point2D *p, long size)
{
    long i;
...
LinearRegression1::LinearRegression1(double *x, double *y, long size)
{
    long i;
    a = b = sumX = sumY = sumXsquared = sumYsquared = sumXY = 0.0;
    n = 0L;
...
void LinearRegression1::addXY(const double& x, const double& y)
{
    n++;
    sumX += x;
    sumY += y;
...
void LinearRegression1::Calculate()
{
    if (haveData())
    {
...
ostream& operator<<(ostream& out, LinearRegression1& lr)
{
    if (lr.haveData())
        out << "f(x) = " << lr.getA()
            << " + ( " << lr.getB()
